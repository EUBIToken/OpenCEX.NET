...
namespace jessielesbian.OpenCEX
{
	...
	public sealed class BlockchainManager{
		...
		public WalletManager GetWalletManager(string privateKey){
			...
			return pool.GetOrAdd(hash, (string disposed) =>
			{
				...
				return new WalletManager(this, new Web3(account, node).Eth, string.Intern(account.Address));
			});
		}
	...
	public sealed class WalletManager{
		...
		public ulong SafeNonce(SQLCommandFactory sqlCommandFactory){
			...
			ulong nonce = (ulong)blockchainManager.SendRequestSync<int>(ethApiContractService.Transactions.GetTransactionCount.BuildRequest(address, StaticUtils.latestBlock));
		...
		{
			string ret = StaticUtils.Await2(etherTransferService.TransferEtherAsync(to, amount.GetAmount2(), gasPrice.Div(StaticUtils.gwei).GetAmount2(), gas.bigInteger, nonce));
