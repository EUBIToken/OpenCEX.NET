		...
		private sealed class Login : RequestMethod{
			...
			public override object Execute(Request request)
			{
				...
				if (throws is null)
				{
					...
					request.httpListenerContext.Response.AddHeader("Set-Cookie", "__Host-OpenCEX_session=" + WebUtility.UrlEncode(Convert.ToBase64String(SessionToken)) + (remember ? ("; Domain=" + CookieOrigin + "; Max-Age=2592000; Path=/; Secure; HttpOnly; SameSite=None") : ("; Domain=" + CookieOrigin + "; Path=/; Secure; HttpOnly; SameSite=None")));
					return null;
				}
